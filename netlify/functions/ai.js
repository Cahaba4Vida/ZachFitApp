...
exports.handler = withErrorHandling(async (event) => {
  ...
  console.log("AI auth header present:", hadAuthHeader);
  const { user, error: authError } = requireAuth(event);
  const hadUser = Boolean(user);
  console.log("AI auth verified:", hadUser);
  if (authError) {
    if (process.env.DEBUG_AUTH === "1") {
      const body = JSON.parse(authError.body);
      return json(authError.statusCode, { ...body, hadAuthHeader, hadUser });
    }
    return authError;
  }
  ...
  if (process.env.DEBUG_AUTH === "1") {
    responseBody.hadAuthHeader = hadAuthHeader;
    responseBody.hadUser = hadUser;
  }
