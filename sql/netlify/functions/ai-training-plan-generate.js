...
`;
  ...
  try {
    ...
    const programLengthWeeks = 4;
    const version = 1;

    // Upsert current program
    await query(
      `insert into training_programs (user_id, program_text, program_json, inputs, program_length_weeks, version, created_at, updated_at)
       ...
       on conflict (user_id) do update
         set program_text = excluded.program_text,
             ...
             updated_at = now()`,
      [auth.user.userId, text, programJson, inputs, programLengthWeeks, version]
    );

    // Optional: append history row (best-effort; ignore if table not created)
    try {
      await query(
        `insert into training_program_history (user_id, program_json, version, created_at)
         values ($1, $2, $3, now())`,
        [auth.user.userId, programJson, version]
      );
    } catch {}
