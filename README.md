...
- `DEBUG_AUTH` (optional, set to `1` to include auth debug flags in `/api/ai` responses)
...
3. **Program Chat (logged in)**: Open DevTools → Network, send a refinement message, verify the request includes an `Authorization` header and returns 200.
4. **Program Chat (logged out)**: Log out and click Send; confirm the app prompts login and does not spam 401s.
5. **Workouts**: Open today’s workout, log sets, save; refresh and confirm logs persist.
6. **PRs**: Add multiple PRs and confirm history + chart update.
7. **Settings**: Toggle units and confirm save.
8. **Admin**: Use allowlisted email to open Admin page; verify client detail + coach prompt link.
